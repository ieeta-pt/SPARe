...
def main(dataset_folder, at, threads):
        ...
        for token in pipe(pd.DataFrame([{"qid":0, "query":text.lower()}]))["query"][0]:
            if token in token2id:
                ...
                if token_id is not None:
                    tokens_ids.append(token)
        return Counter(tokens_ids)
    ...
    out_file_name = "pyterrier_pisa_bow"
    if threads>1:
    ...
    with open(f"results/{out_file_name}.csv", "a") as fOut:
        ...
        questions_dataframe = pd.DataFrame([{"qid":question_ids[i], "query_toks":tokenizer(question.lower())} for i,question in enumerate(questions)])
